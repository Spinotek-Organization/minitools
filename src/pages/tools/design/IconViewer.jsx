...
export default function IconViewer() {
    ...
    const handleFileUpload = (e) => {
        ...
        if (file && file.type === 'image/svg+xml') {
            ...
            reader.onload = (e) => {
                ...
                // Try to extract initial colors? Too complex. Defaults.
            };
    ...
    useEffect(() => {
        ...
        // Simple Regex replacements for editor
        // Note: This is a basic implementation. A real parser would be better but "String replacement" was suggested.
        
        // 1. Optimize (Remove comments, metadata)
        if (optimized) {
        ...
        // 2. Apply Styles
        // We will try to inject style into the root specific attributes if they exist, or add them.
        // Actually, easiest valid way is to wrap contents in a group <g fill="..." ...> or modify valid attributes.
        // But let's try modifying the root <svg> tag attributes.
        
        // Remove existing fill/stroke/stroke-width from root if present to override?
        // Let's just add/replace.
        
        // Helper to replace/add attribute on first <svg> tag
        ...
        // If stroke is none, we might want to remove stroke attribute? 
        // Or just set to none.
        
        // We need to be careful. Some icons rely on internal paths having colors.
        // Force overriding requires simpler regex on ALL elements or CSS injection.
        // Let's use CSS injection for preview?
        // Or simple string replace on specific keywords?
        
        // DESIGN MD SAYS: "Change Fill Color, Stroke Color, Stroke Width"
        
        // Let's use `currentColor` replacement strategy if common, OR just force attributes on root which inherits.
    ...
    return (
        <ToolPageLayout>
            <Helmet>
                <title>SVG Icon Viewer & Editor | MiniTools by Spinotek</title>
                <meta name="description" content="View, edit colors, and optimize SVG icons directly in the browser." />
            </Helmet>
            ...
            <div className="flex flex-col md:flex-row items-center justify-between mb-8 gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-black text-slate-900">SVG Icon Editor</h1>
                        <p className="text-slate-500">Inspect, recolor, and optimize SVG files.</p>
                    </div>
                </div>
                {outputSvg && (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="bg-white rounded-3xl border border-slate-100 p-6 space-y-6 h-fit">
                    <div className="border-2 border-dashed border-slate-200 rounded-2xl p-6 text-center hover:bg-slate-50 transition-colors relative">
                        ...
                        <p className="text-sm font-bold text-slate-700">Upload SVG</p>
                    </div>

                    {svgContent && (
                        <div className="space-y-4">
                            <div>
                                <label className="block text-xs font-bold text-slate-500 mb-1 uppercase">Fill Color</label>
                                <div className="flex gap-2">
                            ...
                            <div>
                                <label className="block text-xs font-bold text-slate-500 mb-1 uppercase">Stroke Color</label>
                                <div className="flex gap-2">
                            ...
                            <div>
                                <label className="block text-xs font-bold text-slate-500 mb-1 uppercase">Stroke Width: {strokeWidth}</label>
                            ...
                            <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                                <label className="text-sm font-bold text-slate-700">Optimize (Minify)</label>
                                <input 
                                    type="checkbox" 
                                    checked={optimized} 
