        ...
        <ToolPageLayout toolId="break-even-calc">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
                ...
                <div className="bg-white rounded-3xl border border-slate-100 p-8 shadow-sm h-fit">
                    <h2 className="text-lg font-bold text-slate-900 mb-6">{t('tools.break-even-calc.section.analysis')}</h2>
                    
                    <div className="space-y-6">
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-1">{t('tools.break-even-calc.form.fixed')}</label>
                            <input
                                type="number"
                                value={fixedCosts}
                            ...
                            />
                            <p className="text-xs text-slate-400 mt-1">{t('tools.break-even-calc.form.fixedHint')}</p>
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-1">{t('tools.break-even-calc.form.variable')}</label>
                            <input
                                type="number"
                                ...
                                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 outline-none focus:ring-2 focus:ring-indigo-500"
                            />
                            <p className="text-xs text-slate-400 mt-1">{t('tools.break-even-calc.form.variableHint')}</p>
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-1">{t('tools.break-even-calc.form.price')}</label>
                            <input
                                type="number"
                                value={pricePerUnit}
                        ...
                        {pricePerUnit <= variableCostPerUnit && (
                            <div className="bg-red-50 text-red-600 p-4 rounded-xl text-sm border border-red-100 flex items-center gap-2">
                                ⚠️ {t('tools.break-even-calc.error.priceLow')}
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="space-y-6">
                     <div className="grid grid-cols-2 gap-4">
                        <div className="bg-indigo-600 text-white p-6 rounded-3xl shadow-lg shadow-indigo-200 text-center">
                            <div className="text-indigo-200 text-sm font-medium uppercase mb-1">{t('tools.break-even-calc.result.units')}</div>
                            <div className="text-4xl font-black">
                                {isFinite(breakevenUnits) ? Math.ceil(breakevenUnits) : 0}
                            </div>
                            <div className="text-xs text-indigo-200 mt-1">{t('tools.break-even-calc.result.unitsSub')}</div>
                        </div>
                        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm text-center">
                             <div className="text-slate-500 text-sm font-medium uppercase mb-1">{t('tools.break-even-calc.result.revenue')}</div>
                            <div className="text-2xl font-black text-slate-900">
                                {isFinite(breakevenRevenue) ? formatCurrency(breakevenRevenue) : '$0'}
                            </div>
                             <div className="text-xs text-slate-400 mt-1">{t('tools.break-even-calc.result.revenueSub')}</div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm h-[300px]">
                        <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={chartData} margin={{ top: 5, right: 30, left: 10, bottom: 5 }}>
                                ...
                                <XAxis dataKey="units" stroke="#94a3b8" fontSize={12} label={{ value: t('tools.break-even-calc.chart.xAxis'), position: 'insideBottom', offset: -5 }} />
                                <YAxis stroke="#94a3b8" fontSize={12} tickFormatter={(val) => `$${val/1000}k`} />
                                <Tooltip formatter={(value) => formatCurrency(value)} contentStyle={{ borderRadius: '12px' }} />
                                <Legend />
                                <Line type="monotone" dataKey="revenue" stroke="#10b981" strokeWidth={3} name={t('tools.break-even-calc.chart.revenue')} dot={false} />
                                <Line type="monotone" dataKey="totalCost" stroke="#ef4444" strokeWidth={3} name={t('tools.break-even-calc.chart.totalCost')} dot={false} />
                                <Line type="monotone" dataKey="fixedCost" stroke="#94a3b8" strokeDasharray="5 5" name={t('tools.break-even-calc.chart.fixedCost')} dot={false} />
                            </LineChart>
                        </ResponsiveContainer>
                    </div>
                </div>
